...
router.get('/posts/create-new/:country', (req, res, next) => {
    ...
    res.render('postViews/newPostWithCountry', {
...
router.get('/posts/create-new/:country/:city', (req, res, next) => {
    ...
    // let allTheCities = require ('full-countries-cities').getCities(req.params.country);
    res.render('postViews/newPostWithCity', {
        theCountry: req.params.country,
...
router.get('/posts/edit/:id', (req, res, next) => {
    ...
    Post.findById(req.params.id)
        .then((postFromDB) => {
            res.render('postViews/editPostWithAll', {
                thePost: postFromDB
            })
        })
        .catch((err) => {
...
router.get('/posts/edit/:id/country', (req, res, next) => {
    ...
    Post.findById(req.params.id)
        .then((postFromDB) => {
            res.render('postViews/editPostWithCountry', {
                thePost: postFromDB
            })
        })
        .catch((err) => {
...
router.get('/posts/edit/:id/city', (req, res, next) => {
    ...
    Post.findById(req.params.id)
        .then((postFromDB) => {
            res.render('postViews/editPostWithCity', {
                thePost: postFromDB
            })
        })
        .catch((err) => {
...
router.get('/posts/:country', (req, res, next) => {
    Post.find({
        ...
        .then((allThePosts) => {
            ...
            let allTheCities = require('full-countries-cities').getCities(req.params.country);
            res.render('postViews/postsWithCountry', {
                posts: allThePosts,
                cities: allTheCities,
                country: req.params.country
            })
    ...
    Post.findById(req.body.postID)
        .then((thePost) => {
            ...
            if(thePost.likes.length === 0){
                ...
                thePost.likes.forEach((eachLikeId, i) => {
                    console.log("hihi3");
                    if (eachLikeId.equals(req.user._id)) {
                        thePost.likes.splice(i, 1);
                    } 
                    else {
                        console.log("I'm here")
                        thePost.likes.push(req.user_id);
                    }
                    thePost.save()
                    .then(() => {
                        res.json(thePost);
